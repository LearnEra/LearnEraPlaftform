...
class OrderTest(ModuleStoreTestCase):
    def setUp(self):
        ...
        course = CourseFactory.create(org='org', number=uuid4().hex[:9], display_name='Test Course')
        self.course_key = course.id
        self.other_course_keys = []
        for i in xrange(1, 5):
            self.other_course_keys.append(CourseFactory.create(org='org', number=uuid4().hex[:9], display_name='Test Course {0}'.format(i)).id)
        self.cost = 40
...
class PaidCourseRegistrationTest(ModuleStoreTestCase):
    ...
    def test_generate_receipt_instructions(self):
        ...
        """
        course2 = CourseFactory.create(org='MITx', number=uuid4().hex[:9], display_name='Robot Duper Course')
...
class CertificateItemTest(ModuleStoreTestCase):
    ...
    def test_refund_cert_callback_before_expiration(self):
        ...
        course = CourseFactory.create(org='refund_before_expiration', number=uuid4().hex[:9], display_name='one')
    ...
    def test_refund_cert_callback_before_expiration_email(self):
        ...
        course = CourseFactory.create(org='refund_before_expiration', number=uuid4().hex[:9], run='course', display_name='one')
        course_key = course.id
        many_days = datetime.timedelta(days=60)
    ...
    def test_refund_cert_callback_after_expiration(self):
        ...
        course = CourseFactory.create(org='refund_after_expiration', number=uuid4().hex[:9], display_name='two')
        course_key = course.id
        course_mode = CourseMode(course_id=course_key,
                                 mode_slug="verified",
                                 mode_display_name="verified cert",
